    ...
    addProseMirrorPlugins() {
        ...
        return [
            new Plugin({
                ...
                appendTransaction: (transactions: any, oldState, newState) => {
                    const docChanges =
                        transactions.some(
                            (transaction: any) => transaction.docChanged,
                        ) && !oldState.doc.eq(newState.doc);
                    const filterTransactions =
                        this.options.filterTransaction &&
                        transactions.some((tr: any) => {
                            let _a, _b;
                            return !((_b = (_a = this.options)
                                .filterTransaction) === null || _b === undefined
                                ? undefined
                                : _b.call(_a, tr));
                        });
                    ...
                    changes.forEach(({ newRange }) => {
                        ...
                        newNodes.forEach(({ node, pos }) => {
                            ...
                            const newNode =
                                deleted && duplicatedNewIds.includes(id);
                            if (newNode) {
                                tr.setNodeMarkup(pos, undefined, {
                                    ...node.attrs,
                                    [attributeName]: generateID(),
                        ...
                        const removeId = (fragment: any) => {
                            ...
                            fragment.forEach((node: any) => {
                                ...
                                if (!types.includes(node.type.name)) {
                                    list.push(
                                        node.copy(removeId(node.content)),
                                    );
                                    return;
                                }
                                // remove id
...
registerEditorExtension({
    ...
    createInstance(ctx: EditorContext) {
        if (ctx.config.uniqueId) {
            return UniqueID.configure(ctx.config.uniqueId);
        }
