    ...
    >
        <div class="github-enterprise-modal">
            ...
            <div class="github-enterprise-modal__body">
                <p>
                    Connect your Github.com account using a personal access
                    token. Create a token on Github.
                    <a :href="tokenUrl" target="_blank">Configure token</a>
                </p>
                <p>
...
@Component({
    name: 'GithubAccessTokenModal',
    components: { CButton, CInput },
})
export default class GithubAccessTokenModal extends Vue {
    token: string = '';
    ...
    get tokenUrl() {
        return `https://github.com/settings/tokens/new?description=acreom&scopes=repo,read:org,read:user,user:email`;
    }

    connect() {
        this.$emit('connect', this.token);
    }
