...
@Component({
    name: 'JiraOnboardingProjectSelectorModal',
    components: {
        ADropDown,
        CInput,
        CButton,
...
export default class JiraOnboardingProjectSelectorModal extends Vue {
    @Prop({ required: true })
    integrationId!: any;
    ...
    async handleCancel(close: any) {
        await this.$store.dispatch('integration/delete', this.jiraIntegration);
        close();
    }
