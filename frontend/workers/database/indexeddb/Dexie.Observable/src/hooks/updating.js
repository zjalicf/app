...
export default function initUpdatingHook(db, tableName, onChange) {
    return function updatingHook(mods, primKey, oldObj, trans) {
        ...
        if (anythingChanged) {
            ...
            const promise = db._changes.add(change); // Just so we get the correct revision order of the update...
