export default function initOverrideCreateTransaction(db, wakeupObservers) {
    return function overrideCreateTransaction(origFunc) {
        return function (mode, storenames, dbschema, parent) {
            ...
            if (
                mode === 'readwrite' &&
                storenames.some(function (storeName) {
                    return (
                        dbschema[storeName] && dbschema[storeName].observable
                    );
                })
            ) {
                ...
                trans.on('complete', function () {
                    if (trans._lastWrittenRevision) {
                        ...
                        if (!parent) {
                            ...
                            wakeupObservers.timeoutHandle = setTimeout(
                                function () {
                            ...
                            var rootTransaction = (function findRootTransaction(
                                trans,
                            ) {
                                return trans.parent
                                    ? findRootTransaction(trans.parent)
                                    : trans;
                            })(parent);
