    ...
    return function syncableConnect(
        ...
        if (db.isOpen()) {
            ...
            if (db._localSyncNode.isMaster) {
                ...
                console.log('We are not master node', packageId);
            ...
            const promise = new Promise$1(function (resolve, reject) {
                db.on('ready', function () {
                    ...
                    return db._syncNodes.get({ url }, function (node) {
                        ...
                        const connectPromise = db.syncable.connect(
                            protocolName,
                            url,
                            options,
                        );
            ...
            return db._syncNodes
                ...
                .first(function (node) {
                    ...
                    if (node) {
                        ...
                        const PersistedContext_1 = initPersistedContext(node);
                        Dexie.Promise.resolve(
                            ...
                            db._syncNodes.add(node).then(function (nodeID) {
                                node.syncContext = new PersistedContext_1(
                                    nodeID,
                                ); // Update syncContext in db with correct nodeId.
                                db._syncNodes.put(node);
                            });
                        });
                    }
    ...
    return function enqueue(context, fn, instanceID) {
        function _enqueue() {
            if (!context.ongoingOperation) {
                ...
                context.ongoingOperation = context.ongoingOperation.then(
                    function () {
...
function bulkUpdate(table, changes) {
    const keys = changes.map(function (c) {
        return c.key;
    });
    const map = {};
    ...
    return table
        ...
        .then(function () {
            ...
            const updatesThatApply = changes.filter(function (c) {
            ...
            const objsToPut = updatesThatApply.map(function (c) {
                const curr = map[c.key + ''];
...
function initApplyChanges(db) {
    return function applyChanges(changes) {
        ...
        const table_names = Object.keys(collectedChanges);
        const tables = table_names.map(function (table) {
        ...
        return db.transaction('rw', tables, function () {
            table_names.forEach(function (table_name) {
                ...
                const createChangesToApply =
                    collectedChanges[table_name][CREATE];
                const deleteChangesToApply =
                    collectedChanges[table_name][DELETE];
                const updateChangesToApply =
                    collectedChanges[table_name][UPDATE];
...
function initFinallyCommitAllChanges(db, node) {
    ...
    return function finallyCommitAllChanges(changes, remoteRevision) {
        ...
        const tablesToIncludeInTrans = db.tables.filter(function (table) {
            return (
                table.name === '_changes' ||
            ...
            return db._changes
                ...
                .then(function (lastChange) {
                    ...
                    if (node.remoteBaseRevisions.length > 1) {
                        for (
                            let i = node.remoteBaseRevisions.length - 1;
                            i > 0;
                            --i
                        ) {
                            if (
                                node.myRevision >=
                                node.remoteBaseRevisions[i].local
                            ) {
...
function getBaseRevisionAndMaxClientRevision(node) {
    ...
    for (let i = node.remoteBaseRevisions.length - 1; i >= 0; --i) {
...
function combineCreateAndUpdate(prevChange, nextChange) {
    const clonedChange = Dexie.deepClone(prevChange); // Clone object before modifying since the earlier change in db.changes[] would otherwise be altered.
...
function initGetChangesSinceRevision(db, node, hasMoreToGive) {
    return function getChangesSinceRevision(
        ...
        let numChanges = 0;
        let partial = false;
        const ignoreSource = node.id;
        let nextRevision = revision;
        return db
            .transaction('r', db._changes, function () {
                const query = db._changes
                    .where('rev')
                ...
                return query
                    ...
                    .each(function (change) {
                        ...
                        if (change.type === CREATE) {
                            ...
                            changeToSend.oldObj = change.oldObj;
                        }
                        if (change.type === DELETE) {
                            changeToSend.oldObj = change.oldObj;
                        }
                        const id = change.table + ':' + change.key;
                        const prevChange = changeSet[id];
                        if (!prevChange) {
            ...
            .then(function () {
                const changes = Object.keys(changeSet).map(function (key) {
            ...
            .then(function () {
                if (limitReached) {
                    ...
                    if (state.tablesToUpload.length === 0) {
                        ...
                        const brmcr = getBaseRevisionAndMaxClientRevision(node);
...
function initGetLocalChangesForNode(db, hasMoreToGive, partialsThreshold) {
    const MAX_CHANGES_PER_CHUNK = partialsThreshold;
    return function getLocalChangesForNode(node, cb) {
        ...
        const getChangesSinceRevision = initGetChangesSinceRevision(
            db,
            node,
            hasMoreToGive,
        );
        const getTableObjectsAsChanges = initGetTableObjectsAsChanges(
            db,
            node,
            MAX_CHANGES_PER_CHUNK,
            getChangesSinceRevision,
        ...
        if (node.myRevision >= 0) {
            ...
            if (node.dbUploadState === null) {
                ...
                const tablesToUpload = db.tables
...
function initConnectProtocol(
    ...
    const hasMoreToGive = { hasMoreToGive: true };
    ...
    return function connectProtocol(node, activePeer) {
        /// <param name="node" type="db.observable.SyncNode"></param>
        const getLocalChangesForNode = initGetLocalChangesForNode(
            db,
            hasMoreToGive,
        ...
        function sendChangesToProvider(
            ...
            const finalSyncPromise = new Promise$2(function (resolve, reject) {
        ...
        function applyRemoteChanges(
            ...
            const saveToUncommittedChanges = initSaveToUncommittedChanges(
                db,
                node,
            );
            const finallyCommitAllChanges = initFinallyCommitAllChanges(
                db,
                node,
            );
            return enqueue(
                ...
                function () {
                    if (!stillAlive())
                        return Promise$2.reject(
                            new Dexie.DatabaseClosedError(),
                        );
                    // FIXTHIS: Check what to do if clear() is true!
                    return (
                        partial
                            ? saveToUncommittedChanges(
                                  remoteChanges,
                                  remoteRevision,
                              )
                            : finallyCommitAllChanges(
                                  remoteChanges,
                                  remoteRevision,
                              )
        ...
        function continueUsingReactPattern(continuation) {
            ...
            function reactToChanges() {
                ...
                getLocalChangesForNode_autoAckIfEmpty(
                    node,
                    function (
                        changes,
                        remoteBaseRevision,
                        ...
                        if (changes.length > 0) {
                            continuation.react(
                                ...
                                partial,
                                function onChangesAccepted() {
                                    Object.keys(nodeModificationsOnAck).forEach(
                                        function (keyPath) {
                                            Dexie.setByKeyPath(
                                                node,
                                                keyPath,
                    ...
                    console.error(
                        'Got ' + ex.message + ' caught by reactToChanges',
                    );
                    abortTheProvider(ex);
                });
            }
...
function initConnectFn(db, activePeers) {
    return function connect(
        ...
        const existingPeer = activePeers.filter(function (peer) {
            return peer.url === url;
        });
        ...
        function execConnect() {
            ...
            // connectProtocol
            const rejectConnectPromise = { p: null };
            const connectProtocol = initConnectProtocol(
                db,
                protocolInstance,
                dbAliveID,
                options,
            ...
            const getOrCreateSyncNode = initGetOrCreateSyncNode(
                db,
                protocolName,
                url,
            );
            const connectPromise = getOrCreateSyncNode(options).then(function (
                node,
            ) {
            ...
            var activePeer = {
                ...
                disconnect(newStatus, error) {
                    const pos = activePeers.indexOf(activePeer);
...
const Observable = Dexie.Observable;
...
function Syncable(db) {
    ...
    db.syncable.connect = function (protocolName, url, options) {
        ...
        const protocolInstance =
            Dexie.Syncable.registeredProtocols[protocolName];
        if (protocolInstance) {
            return syncableConnect(
                protocolInstance,
                protocolName,
                url,
                options,
        ...
        return new Promise(resolve => {
            db.transaction(
                ...
                async () => {
                    ...
                    // accordingly.
                    const syncNodes = await db._syncNodes
                        .where('url')
                        .equals(url)
                        .toArray();
                    const nodeIDsToDelete = syncNodes.map(node => node.id);
...
if (Dexie.Syncable) {
    ...
    Syncable.registerSyncProtocol = Dexie.Syncable.registerSyncProtocol;
    Syncable.unregisterSyncProtocol = Dexie.Syncable.unregisterSyncProtocol;
    Syncable.registeredProtocols = Dexie.Syncable.registeredProtocols || {};
} else {
    ...
    Dexie.Syncable.registerSyncProtocol = function (name, protocolInstance) {
        ...
        if (typeof partialsThreshold === 'number') {
            ...
            if (isNaN(partialsThreshold) || partialsThreshold < 0) {
                throw new Error(
                    'The given number for the threshold is not supported',
                );
            }
    ...
    Dexie.Syncable.unregisterSyncProtocol = function (name) {
        delete Syncable.registeredProtocols[name];
    };
    Dexie.addons.push(Syncable);
}
const Dexie_Syncable = Dexie.Syncable;

export default Dexie_Syncable;
